"""
models/classification/resnet.py

ResNet-18 training module for FracAtlas binary fracture classification.

Follows the same interface as models/yolo/train.py:
    run_training(config: dict) -> Path

Supported experiments
---------------------
    E4a  — Class weight tuning         (loss=weighted_ce, weight_mult)
...
-----------------------
    experiment_id   str     e.g. "E4a_m050"
...
FRAC_CLASS    = "Fractured"   # ImageFolder alphabetical order → index 0
...
class FocalLoss(nn.Module):
    ...
    Args:
        gamma:     Focusing exponent. 0 = weighted CrossEntropy.
                   1 → mild focus (recommended first try for medical imaging).
                   2 → strong focus (Lin et al. 2017 default).
        weight:    Per-class alpha weights, same as CrossEntropyLoss.weight.
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        pt = torch.exp(-ce)                        # p_t = e^(-CE) = probability of true class
        fl = (1.0 - pt) ** self.gamma * ce         # down-weight easy examples
...
class WarmupCosineScheduler:
    ...
    - Epochs 1 .. warmup_epochs : LR scales linearly  0 → base_lr
    - Epochs warmup+1 .. total  : LR decays smoothly  base_lr → min_lr
...
def _compute_class_weights(
    ...
    """Derive per-class weights from training set label counts.

    weight_mult=0.0 → flat weights [1.0, 1.0]  (ignore imbalance)
    weight_mult=1.0 → natural imbalance ratio   (E3 level)
...
def _evaluate(
    ...
    """Standard single-pass evaluation. Returns metric dict."""
    model.eval()
...
def _evaluate_tta(
    ...
    """TTA evaluation: average probabilities over N augmented passes."""
    model.eval()
...
def run_training(config: dict) -> Path:
    """Run ResNet-18 classification training from a config dict.
