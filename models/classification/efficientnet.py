"""
models/classification/efficientnet.py

EfficientNet-B3 training module for FracAtlas binary fracture classification.

Follows the same interface as models/classification/densenet.py:
    run_training(config: dict) -> Path

Supported experiments
---------------------
    F1  — Baseline: plateau scheduler, flat LR, 50 epochs (mirrors D1)
    F2  — Cosine warmup (mirrors D2 / E4e)
    F3  — CLAHE all splits (mirrors E5 / D3)
    F4  — CLAHE + AlbumentationsDelta train-only (mirrors E6 / D4)

Architecture note
-----------------
EfficientNet-B3 uses compound scaling: depth, width, and input resolution
are scaled jointly by a single coefficient (φ=1.2 for B3). This differs from
ResNet's depth-only scaling and DenseNet's dense connectivity — B3 achieves
higher accuracy at comparable parameter counts (~12M vs DenseNet-169 14M).
Head: classifier = Sequential(Dropout(0.3), Linear(1536→2)).
GradCAM target: "features.8" (final Conv2dNormActivation before GAP).

Config keys (from YAML)
-----------------------
    experiment_id   str     e.g. "F1"
    task            str     must be "classify_efficientnet"
    ...
    use_clahe           bool    CLAHE local contrast enhancement on all splits (default False)
    use_albu            bool    AlbumentationsDelta on train only (default False)
    use_augmix          bool    XRayAugMix on train only (default False)
    early_stop_patience int     stop if no val F1 improvement for N epochs; 0=disabled (default 0)
...
def _build_model(dropout_p: float, device: torch.device) -> nn.Module:
    """EfficientNet-B3 (ImageNet pretrained), all layers unfrozen, optional dropout.

    Original head: classifier = Sequential(Dropout(0.3), Linear(1536→1000)).
    Replaced with Linear(1536→2) or Sequential(Dropout, Linear(1536→2)).
...
def _get_transforms(
    ...
    # non-CLAHE images). TTA is disabled in all experiments — do not use it.
    tta_tfs = [
        val_tf,
        transforms.Compose([
            transforms.Resize((img_size, img_size)),
...
def run_training(config: dict) -> Path:
    """Run EfficientNet-B3 classification training from a config dict.
    ...
    # ── Experiment label ──────────────────────────────────────────── #
    experiment = (
        f"{exp_id} | EfficientNet-B3 | classify"
        f" | epochs={epochs}"
        f" | loss={loss_type}"
    ...
    logger.log_start({
        ...
        "Model"    : f"EfficientNet-B3 (ImageNet pretrained)  |  in_features: 1536  |  dropout: {dropout_p}",
    ...
    train_tf, val_tf, tta_tfs = _get_transforms(
        img_size, use_clahe=use_clahe, use_albu=use_albu, use_augmix=use_augmix
    )
